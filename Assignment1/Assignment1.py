...
class TrigStations(object):
    def __init__(self,Long,Lat,r,LoBand):
        self.Lat = mt.radians(Long[0] + Long[1]/60 + Long[2]/3600)

        self.Long = mt.radians(Lat[0] + Lat[1]/60 + Lat[2]/3600)
    ...
    for name,data in TrignetStations.items():
        ...
        for n in range(2,10):
            ...
            for m in range(0,n):
                ...
                pnm = round(sympy.assoc_legendre(n,m,mt.cos(data.Lat)),10)

                val = (dCnm * mt.cos(m*data.Long) + Snm * mt.sin(m*data.Long))*pnm
